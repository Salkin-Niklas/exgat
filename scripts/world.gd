...
func _process(delta: float) -> void:
	...
	if Vector2.UP.dot(dir)>0:
		y = player.global_position.y -viewport_rect.y/2+190
	else:
		y = player.global_position.y +viewport_rect.y/2-190
	var x: float = 0
	if Vector2.RIGHT.dot(dir)>0:
		x = player.global_position.x +viewport_rect.x/2-190
	else:
		x = player.global_position.x -viewport_rect.x/2+190
	...
	if is_nan(pos.x):
		pos.x = 0
	var bb: Rect2 = Rect2(player.global_position.x -viewport_rect.x/2+190, player.global_position.y -viewport_rect.y/2+190, viewport_rect.x-379, viewport_rect.y-379)
	if !bb.has_point(pos):
		pos = Vector2(x,m*x+b)
	
	$GUI/station_direction.position = $GUI/station_direction.position.lerp(pos-player.global_position+viewport_rect/2, delta*4)
	$GUI/station_direction/Sprite2D.rotation = (pos-player.global_position).bounce(Vector2.RIGHT).angle_to(Vector2.UP)
	
	if (pos-player.global_position).length()>(dir.length()-380):
