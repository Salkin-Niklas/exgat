...
func _process(delta: float) -> void:
	...
	if Vector2.UP.dot(dir)>0:
		y = player.global_position.y -viewport_rect.y/2+100
	else:
		y = player.global_position.y +viewport_rect.y/2-100
	var x: float = 0
	if Vector2.RIGHT.dot(dir)>0:
		x = player.global_position.x +viewport_rect.x/2-100
	else:
		x = player.global_position.x -viewport_rect.x/2+100
	...
	if is_nan(pos.x):
		pos.x = 0
	var bb: Rect2 = Rect2(player.global_position.x -viewport_rect.x/2+100, player.global_position.y -viewport_rect.y/2+100, viewport_rect.x-199, viewport_rect.y-199)
	if !bb.has_point(pos):
		pos = Vector2(x,m*x+b)
	...
	if (pos-player.global_position).length()>(dir.length()-250):
